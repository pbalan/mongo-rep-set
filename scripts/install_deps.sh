...
# base
apt-get update
apt-get install -y --no-install-recommends ca-certificates openssl numactl wget vim
...
echo "deb http://repo.mongodb.org/apt/debian jessie/mongodb-org/$MONGO_MAJOR main" > /etc/apt/sources.list.d/mongodb-org-3.6.list
